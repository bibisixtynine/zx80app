   


---------------
Qwark Alpha 0.2
---------------
La boîtes à outils pour créer et
partager des App récréatives
non addictives... pour les gamins et
gamines de 7 à 77 ans !


Quelques instructions :
-----------------------
  
1) Choisi une App avec le bouton nuage fleche
vers le bas, puis appuye sur bouton de
selection vert pour choisir une App

2) Lance l'App avec le bouton play, qui 
correspond au triangle qui pointe à droite

3) Reviens au code en appuyant 
sur le bouton <\> en bas à gauche

4) Sauvegarde tes changements avec 
le bouton nuage fleche vers le haut
dans le cloud

5) Pour créer une nouvelle App à partir d'une
App existante, duplique le code de l'App courante avec
le bouton + et donne lui un nouveau nom

Tu peux aussi aller dans un autre dossier 
que celui où sont rangées tes Apps, avec
le bouton en forme d'engrenage... mais
attention à ne pas modifier le dossier de
quelqu'un d'autre !

big bisous de Bibi69 !